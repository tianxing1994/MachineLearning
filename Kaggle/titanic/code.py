...
# <-------- 样本数据字符转数字 start-------->
#  Name, Sex, Ticket, Cabin, Embarked 是文本, 需要转化成数字.
test = test_origin.copy()
Name_test_dict = dict(zip(test.loc[:,"Name"].unique(),range(len(test.loc[:,"Name"].unique()))))
...
test["Name"] = test["Name"].map(Name_test_dict)
...
Name_train_dict = dict(zip(train.loc[:,"Name"].unique(),range(len(train.loc[:,"Name"].unique()))))
...
test["Name"] = test["Name"].map(Name_train_dict)
...
# 得到 Name, Sex, Ticket, Cabin, Embarked 字段被转为数字的 DataFrame.
# <-------- 样本数据字符转数字 start-------->

train_column_isnull_any_condition = []
train_column_isnull_sum_condition = []
for column in train.columns:
    train_column_isnull_any_condition.append(train.loc[:,column].isnull().any())
    train_column_isnull_sum_condition.append(train.loc[:,column].isnull().sum())
